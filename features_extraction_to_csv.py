...
def return_128d_features(path_img):
    ...
    # 因为有可能截下来的人脸再去检测，检测不出来人脸了, 所以要确保是 检测到人脸的人脸图像拿去算特征 / For photos of faces saved, we need to make sure that we can detect faces from the cropped images
    if len(faces) != 0:
...
def main():
    ...
    with open("data/features_all.csv", "w", newline="") as csvfile:
        ...
        for person in person_list:
            # 得到face/personX的平均特征，它将是一个长度为128D的列表 / Get the mean/average features of face/personX, it will be a list with a length of 128D
            logging.info(path_images_from_camera, person)
            ...
            # features_mean_personX将是129D，人名+ 128个特征 / features_mean_personX will be 129D, person name + 128 features
